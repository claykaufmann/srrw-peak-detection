...
def detect_stage_rises(data: np.ndarray) -> np.ndarray:
    ...
    with open('../Anomaly_Detection/Hyperparameters/Stage/detect_stage_rises_params.pkl', 'rb') as pck_file:
...
def outdated_detect_stage_rises(data : np.ndarray) -> np.ndarray:
    ...
    small_window_size = 3
    threshold = .03
    flat_thresh = .001 # used to determine if points are too flat
    r = 2 # rounding factor
    signals = np.zeros(len(data)) 
    ...
    for i in range(len(data) - 3):
        if abs(data[i+2] - data[i+1]) < flat_thresh and abs(data[i+1] - data[i]) < flat_thresh:
            signals[i+1] = 0
    ...
    for i in range(1,len(signals)-2):
        ...
        if signals[i] == 1 and signals[i-1] == 0 and signals[i+1] == 1 and signals[i+2] == 0:
            signals[i] = 0
        if signals[i] == 1 and signals[i+1] == 0 and signals[i+2] == 1 and not(data[i] > data[i+1] and data[i+1] > data[i+2]):
            signals[i+1] = 1
    return signals
...
def read_in_ground_truths_data(filename: str, is_julian: bool = True) -> list[list]:
...
def write_data_to_trainset(
    ...
    *_data:         timeseries
    ...
    if is_julian_time:
        fDOM_data = [
            [dp.julian_to_datetime(entry[0]), entry[1], entry[2]] for entry in fDOM_data
        ]
        stage_data = [
            [dp.julian_to_datetime(entry[0]), entry[1], entry[2]]
            for entry in stage_data
        ]
        turb_data = [
            [dp.julian_to_datetime(entry[0]), entry[1], entry[2]] for entry in turb_data
        ]
    ...
    for i in range(len(fDOM_data)):
        ...
        if not (fDOM_time == stage_filtered[i][0] == turb_filtered[i][0]):
            raise ValueError(
                "Data align failed at idx: {} ,with fDOM: {}, stage: {}, turb: {}".format(
                    i, fDOM_time, stage_filtered[i][0], turb_filtered[i][0]
                )
            )
    ...
    with open(out_file, "w", newline="") as outfile:
        ...
        for i in range(len(fDOM_data)):
            ...
            if i >= data_start and i < data_end:
                ...
                writer.writerow(
                    ["fDOM", fDOM_time, f"{fDOM_data[i][1]:.5f}", fDOM_data[i][2]]
                )
                writer.writerow(
                    ["Stage", stage_time, f"{stage_data[i][1]:.5f}", stage_data[i][2]]
                )
                writer.writerow(
                    ["Turbidity", turb_time, f"{turb_data[i][1]:.5f}", turb_data[i][2]]
                )
    outfile.close()


# Specifically for plotting annotated data to the 'Trainset' website plotting software
def write_detected_to_trainset(
    fDOM_events: list[np.ndarray],
    ...
    *_data:     timeseries data
    ...
    fDOM_merged = dp.merge_data(
        fDOM_data, fDOM_events, flags["fDOM_flag"], flags["default_fDOM_flag"]
    )
    stage_merged = dp.merge_data(
        stage_data, stage_events, flags["stage_flag"], flags["default_stage_flag"]
    )
    turb_merged = dp.merge_data(
        turb_data, turb_events, flags["turb_flag"], flags["default_turb_flag"]
    )

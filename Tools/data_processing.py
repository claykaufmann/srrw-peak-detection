...
def datetime_to_julian(date: datetime.datetime) -> float:
    ...
    jdn = (
        date.day
        + math.floor((153 * interm3 + 2) / 5)
        + 365 * interm2
        + math.floor(interm2 / 4)
        - math.floor(interm2 / 100)
        + math.floor(interm2 / 400)
        - 32045
    )
...
def interpolate_missing_intervals(data: np.ndarray) -> np.ndarray:
    ...
    for i in range(data.shape[0] - 1):
        ...
        if julian_to_datetime(data[i, 0]) + delta15 != julian_to_datetime(
            data[i + 1, 0]
        ):
            ...
            diff = int(
                (
                    (
                        (
                            julian_to_datetime(data[i + 1, 0])
                            - julian_to_datetime(data[i, 0])
                        ).total_seconds()
                    )
                    / 900
                )
            ...
            if diff < 5:
                ...
                for j in range(i, i + diff):
                    ...
                    insert_vals.append(
                        [
                            datetime_to_julian(
                                julian_to_datetime(data[i, 0]) + increment_timedelta
                            ),
                            data[i, 1] + ((k) * increment),
                        ]
                    )
...
def remove_5_min_intervals(
    data: np.ndarray, is_julian: bool = False, convert_to_array: bool = False
):
    """
    ...
    if is_julian:
        ...
        data = [
            [datetime.datetime.strp(entry[0], "%Y-%m-%d %H:%M:%S"), entry[1]]
            for entry in data
        ]
...
def merge_data(
    data: np.ndarray, events: list[np.ndarray], flag: str, default_flag: str
) -> list[list]:
    """
...
def interpolate_timeseries(
    data: np.ndarray, pks: np.ndarray, locs: np.ndarray, w: np.ndarray, p: np.ndarray
) -> np.ndarray:
    """
    ...
    for i in range(locs.shape[1]):
        try:
            ...
            data[start_index : end_index + 1, 1] = np.linspace(start_val, end_val, len)
        except:
            pass
...
def smooth_data(data, passes):
    ...
    for n in range(passes):  # if passes == 0, then no smoothing will be applied
        for i in range(1, len(data) - 1):
            data[i] = (
                data[i - 1] + 2.0 * data[i] + data[i + 1]
            ) / 4.0  # weight unsmoothed value 2x
    return data
...
def find_turning_points(data):
    ...
    turning_points = np.hstack(
        (idx + 1, sign_diffs[idx])
    )  # stack index of turning points with sign differences
...
def compute_event_info(event: np.ndarray) -> dict:
    ...
    struct["centroid"] = sum(np.multiply(event[:, 0], event[:, 1])) / sum(event[:1])
    return struct
...
def delete_missing_data_events(
    events: list[np.ndarray], missing_file_path: str
) -> list[np.ndarray]:
    """
    ...
    with open(missing_file_path, newline="") as file:
        ...
        for event in events:
            ...
            for pair in time_list:
                if (pair[0] < start and start < pair[1]) or (
                    pair[0] < end and end < pair[1]
                ):
                    flag = False
                    break
            if flag:
...
def delete_missing_data_peaks(
    data: np.ndarray, peaks: list, props: dict, missing_file_path: str
) -> tuple:
    """
